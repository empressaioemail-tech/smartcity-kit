...
At G-102 the copies were re-vendored across eight product commits, from `c7d7980` to `fd52428`, and the denominator DID NOT MOVE. The rule was run, not assumed: 143 before, 143 after, zero classes added and zero removed, both sides measured by running `stylesheetClasses()` over the two stylesheets rather than one being derived from the other. `shell.css` grew by 1281 bytes over that span and every edit in it is invisible to this rule by construction, which is worth stating because a stylesheet that grows while the vocabulary holds still reads like a stale pin until it is measured. G-95 added a comment block, G-95 dropped `h5` from the `.state` heading selector, and G-97 R1 replaced one id selector with another. A comment, an element selector and two id selectors. None of the three is a class. What the re-vendor DID move was markup parity, on two cases, which is where the work landed.
