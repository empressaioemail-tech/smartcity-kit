...
- **Preview compositions come from `examples/gallery.tsx`**, which is maintained in the kit and covers all 73 components with real fixture data. If a component's API changes, its GALLERY entry is the thing to update first — the previews port from it.
- Two of the five components `30b` section 3.1 calls load-bearing — the applicability matrix (`mx*`) and the code citation (`cite`) — have no CSS in any shipped stylesheet, so the kit cannot wrap them and the design agent will invent them. Plan Review surfaces are the gap. Not fixable in this repo; the product must ship the CSS first.
