...
- **G-102 added no family and no component.** The copies were re-vendored across eight product commits and the vocabulary held at 143, so the count here is still 86 components, 86 gallery entries and 86 previews. What changed is two component CONTRACTS. `Prov` can now carry a second figure through `secondClaim`, and the second figure cannot arrive without its own counting rule; the two figures paint together at the head of the chip and the two rules together at the tail, which is the order the product serves. `State` now emits an `h2` rather than an `h5`, because every absence block sits directly under the page `h1` and the product dropped `h5` from the stylesheet on purpose so a re-introduced level skip renders unstyled. A test reads the heading element out of the vendored stylesheet and asserts the component matches it, so the next re-vendor that moves the level goes red here rather than silently.
